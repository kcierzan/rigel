...
fn test_apply_gain() {
    ...
    for i in 0..64 {
        assert_eq!(output.samples[i], 2.0);
    }
}
```
...
fn test_soft_clip() {
    ...
    for i in 0..64 {
        input.samples[i] = 10.0; // Well beyond [-1, 1]
    }
    ...
    for i in 0..64 {
        assert!(output.samples[i] > 0.99 && output.samples[i] < 1.0);
    }
}
```
...
fn test_exponential_decay() {
    ...
    assert!((output.samples[0] - 1.0).abs() < 0.01);
    
    // Verify decay (each sample smaller than previous)
    for i in 1..64 {
        assert!(output.samples[i] < output.samples[i - 1]);
    }
}
```
...
fn test_wavetable_oscillator() {
    ...
    for i in 0..64 {
        assert!(output.samples[i] >= -1.0 && output.samples[i] <= 1.0);
    }
}
```
...
fn test_crossfade() {
    ...
    for i in 0..64 {
        buffer_a.samples[i] = 1.0;
        buffer_b.samples[i] = 0.0;
    }
    ...
    for i in 0..64 {
        assert!((output.samples[i] - 0.707).abs() < 0.01);
    }
}
```
...
fn apply_ramped_gain(
    ...
    for i in 0..64 {
        output.samples[i] = input.samples[i] * gain_block.samples[i];
    }
}
...
fn test_parameter_ramp() {
    ...
    assert!(output.samples[0] < 0.02);
    
    // Last sample should be near 1.0
    assert!(output.samples[63] > 0.98);
