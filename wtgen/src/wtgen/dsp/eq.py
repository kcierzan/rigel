...
class Equalizer:
    def __init__(
        self,
        eq_settings: str | None = None,
        high_tilt_settings: str | None = None,
        low_tilt_settings: str | None = None,
        sample_rate: float = 44100.0,
    ):
        self._sample_rate = sample_rate
        self._nyquist = sample_rate / 2.0
        self._eq_settings = eq_settings
        self._high_tilt_settings = high_tilt_settings
        self._low_tilt_settings = low_tilt_settings

    def apply(self, input: np.ndarray) -> np.ndarray:
        """Apply the eq settings to the input and return a new ndarray."""
        processed = input.copy()

        if self._eq_settings:
            eq_bands = self._parse_eq_settings(self._eq_settings)
            processed = self._apply_parametric_eq_fft(
                processed, eq_bands, preserve_rms=True, preserve_phase=True
            )

        if self._low_tilt_settings:
            start_ratio, gain_db = self._parse_tilt_settings(self._low_tilt_settings)
            processed = self._apply_tilt_eq_fft(processed, start_ratio, gain_db, tilt_type="low")

        if self._high_tilt_settings:
            start_ratio, gain_db = self._parse_tilt_settings(self._high_tilt_settings)
            processed = self._apply_tilt_eq_fft(processed, start_ratio, gain_db, tilt_type="high")

        return processed

    def _parse_tilt_settings(self, tilt_string: str) -> tuple[float, float]:
        start_ratio, gain_db = (float(part) for part in tilt_string.split(":"))
        return start_ratio, gain_db
    ...
    def _create_eq_band(
        ...
        """
        if not 0.0 < frequency_hz < self._nyquist:
            raise ValueError(
                f"Frequency must be between 0.0 and {self._nyquist} Hz, got {frequency_hz}"
            )

        # Convert Hz to normalized frequency (ratio of Nyquist)
        frequency_normalized = frequency_hz / self._nyquist
    ...
    def _apply_parametric_eq_fft(
        ...
        for band in eq_bands:
            ...
            if "frequency_hz" in band:
                freq_norm = band["frequency_hz"] / self._nyquist
            else:
    ...
    def _parse_eq_settings(self, eq_string: str | None) -> EQSettings:
        """
        ...
        """
        if not eq_string or not eq_string.strip():
            return []
        ...
        try:
            for band_str in eq_string.split(","):
                ...
                eq_band = self._create_eq_band(frequency_hz, gain_db, q_factor)
    ...
    def _create_high_tilt_eq(
        ...
        if not 0.0 <= start_freq_ratio <= 1.0:
            raise ValueError(
                f"start_freq_ratio must be between 0.0 and 1.0, got {start_freq_ratio}"
            )
    ...
    def _apply_tilt_eq_fft(
        ...
        if tilt_type == "high":
            tilt_response = self._create_high_tilt_eq(start_freq_ratio, tilt_db)
        else:
            tilt_response = self._create_low_tilt_eq(start_freq_ratio, tilt_db)
    ...
    def analyze_eq_response(
        ...
        """
        frequencies_hz = np.linspace(0, self._nyquist, num_points)
        frequencies_norm = frequencies_hz / self._nyquist  # Normalized for calculations
        ...
        for band in eq_bands:
            # Handle both old (normalized) and new (Hz) formats for backward compatibility
            if "frequency_hz" in band:
                freq_center_norm = band["frequency_hz"] / self._nyquist
            else:
    ...
    def analyze_tilt_eq_response(
        ...
        """
        frequencies_hz = np.linspace(0, self._nyquist, num_points)
        frequencies_norm = frequencies_hz / self._nyquist
        magnitudes_db = np.zeros(num_points)
